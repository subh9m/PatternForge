...
import CnCheatSheet from './CnCheatSheet';
import CnPlayground from './CnPlayground';
import { cnConcepts } from './cnRegistry';
...
export default function CnMainContent({ isOpen, toggleSidebar, activeTab }) {
  const currentConcept = cnConcepts.find(item => item.id === activeTab);
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-10 pt-10 space-y-8 reading-module-content">
        {activeTab === 'cn_cheat_sheet' ? (
          <CnCheatSheet />
        ) : activeTab === 'cn_playground' ? (
          <CnPlayground />
        ) : currentConcept ? (
          <CnCard key={currentConcept.id} data={currentConcept} />
        ) : (
          <CnCard key={cnConcepts[0].id} data={cnConcepts[0]} />
        )}
      </div>
    </main>
  );
